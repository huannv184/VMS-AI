...
- **Trade-off (closed 2026-05-17 via BUG-WS-GLOBAL-FANOUT-01 fix)**: the per-camera EventManager dispatch ALSO broadcasts to camera 0. Before the fix, a viewer subscribed to camera 0 still received every event from every camera. Closed by broadcast-side filtering in `broadcastEvent`: for non-zero camera_id events fanning out to camera-0 subscribers, the `isSocketAllowedForCamera` helper consults the same `g_allowed_cameras` map (mutex-protected, no Qt property reads from producer threads) and drops out-of-scope recipients. Camera-0 subscribe remains open so the legitimate "viewer wants system notifications" pattern still works; the leak channel is closed at the emitter.
