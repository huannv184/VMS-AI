...
## 2026-05-21 BUG-PPE-PARTIAL-ENFORCEMENT-01 closed — wired all 5 wearable violations (was helmet+vest only)

- **Files**: `cpp-backend/include/utils/config.h` (PpeClassConfig +3 fields `gloves_class=3, mask_class=4, boots_class=5`); `cpp-backend/src/utils/config.cpp` (YAML parse +3 fields); `cpp-backend/config/backend.yaml` (doc block +3 commented fields); `cpp-backend/src/main.cpp` (VMS_AI_PPE_CONFIG_JSON env propagation +3 fields + log line); `cpp-backend/src/ai_worker/main.cpp` (PpeRuntimeConfig +3 fields, collection loop +3 vectors, contained_in lambda gained `min_overlap` param, derivation +3 pairings, emit_violation +3 codes 408/409/410, boot log line +3 fields); `cpp-backend/src/core/ai_event_processor.cpp` (dispatch range `406..410` + 3 label aliases, processPPEViolation +3 Vietnamese descriptions / 3 cooldown kinds).
- **Bug**: PPE engine ships 6 detection classes (helmet/vest/person/gloves/mask/boots, per 2026-05-19 retrain). After the 2026-05-19 PPE pass closed BUG-PPE-PHANTOM-FEATURE, only helmet+vest were paired against persons into NoHelmet (406) / NoVest (407) violation events. The other 3 wearables (gloves/mask/boots) were emitted as raw detections in metadata (class_ids 303/304/305) but no derivation existed → no Event row → AlarmsView / PpeMonitorView silently never saw them. Operator reported "anh chưa phát hiện các áo phát quang, kính bảo hộ hay găng tay" — vests WERE in the spec but PPE engine wasn't enabled in operator's session (separate gap), gloves was the spec gap, glasses was never trained into the engine at all.
- **Root cause**: 3 different problems collapsed under one symptom — (a) PPE engine OFF by default per camera (`ai_config` DB column lacks `"ppe":true`, env `VMS_AI_ENABLE_PPE` not set); (b) Gloves/Mask/Boots had no violation derivation in `ai_worker/main.cpp:1310-1316` even when engine on; (c) Safety glasses isn't one of the 6 trained engine classes — undetectable regardless. This bugfix addresses (b). Operator must still set `VMS_AI_ENABLE_PPE=1` (or add `"ppe":true` to per-camera `ai_config`) for (a); (c) requires engine retrain.
- **Fix**:
  1. **Config layer** — extended `PpeClassConfig` struct with 3 new int fields defaulting to 3/4/5 (matching shipped engine slot order). YAML doc block in `backend.yaml` shows operators how to disable any check per-deployment via `gloves_class: -1` etc. Default = enforce all 5 (operator chose option B "bật hết").
  2. **Propagation** — `main.cpp` extended VMS_AI_PPE_CONFIG_JSON to include all 7 fields (labels + 6 indices). ai_worker's lambda init reads the 3 new fields, defaults preserved if absent.
  3. **Derivation** — `ai_worker/main.cpp` collection loop now bins into 5 wearable vectors. `contained_in` lambda gained `min_overlap` parameter so each wearable can use its own threshold (helmet/vest/boots 0.50, gloves 0.30, mask 0.40). Gloves threshold lower because hands often partly extend beyond person bbox; mask between because face is usually well-contained but smaller. Compliance counter now requires ALL ENABLED checks to pass for `compliant_count++`; any single failure → `violating_count++`. Emits synthetic violations 408=NoGloves, 409=NoMask, 410=NoBoots.
  4. **Dispatch** — `ai_event_processor.cpp:166` widened from `class_id == 406 || class_id == 407` to range `406..410` plus 3 new label aliases. `processPPEViolation` extended with 3 new kinds + Vietnamese descriptions for the existing PpeMonitorView pipeline (which renders `evt.description` verbatim — no frontend code change required).
- **Concurrency notes**: Each ai_worker is per-camera single-threaded; derivation loop is `O(persons × wearables_per_class)` per frame. Adding 3 more wearable types triples the loop work in the worst case but bounded to ≤ few-dozen iterations (PPE engine NMS caps detections). `processPPEViolation` cooldown shape unchanged: per-camera + kind + (track_id OR 50px center bucket); 3 new kinds create 3 new cooldown keys per person, no contention with existing helmet/vest paths.
- **Verification**: `cmake --build` clean both targets (vms_backend + ai_worker_v2). `ctest -C Release` 9/9 pass. End-to-end smoke test deferred — operator runs with `$env:VMS_AI_ENABLE_PPE=1` to verify NoGloves/NoMask/NoBoots events appear in AlarmsView when person walks in without those items.
- **What's NOT closed** (operator-side or scope-deferred):
  - **Safety glasses (kính bảo hộ)** — engine simply doesn't recognize this class. Requires dataset collection + retraining `best.pt` with a `glasses` label, then re-export FP16 via `tools/test_ppe_engine.py` workflow. Out of scope for code-side bugfix; operator action required.
  - **Per-camera enforcement profile** — current YAML config is global (one `ai.ppe.*` block for the whole deployment). A site with mixed indoor/outdoor cameras can't say "indoor camera ignores gloves+boots, outdoor enforces all 5". Workaround today is per-camera `ai_config` JSON override; ideal would be `cameras.*.ppe_overrides` block in YAML.
  - **One-glove edge case**: derivation checks "ANY glove bbox inside person bbox" → person with left-hand glove only counts as compliant. Real spec is usually both hands. Engine doesn't distinguish L/R glove; fixing this needs paired-detection logic or wrist-anchored matching.
- **Detection lesson**: when a "feature not working" report stacks 3 distinct gaps (item not detected because feature off, item not derived because code missing, item not detectable because not trained), separate them in the response before touching code. Operator's frustration in the original message conflated all 3; fixing only one would leave 2 still broken and look like the feature is still dead.

## 2026-05-21 BUG-FE-LOGIN-REMOVECHILD-01 closed — browser auto-translate vs React reconciler

- **Files**: `vms-frontend/index.html` (added `translate="no"` on `<html>` + `<meta name="google" content="notranslate">`).
- **Bug**: `LoginView.jsx` button threw `NotFoundError: Failed to execute 'removeChild' on 'Node': The node to be removed is not a child of this node.` on state transitions (loading flip, step change, error toggle). Stack `at button → at form → at div → at div → LoginView` matched the step 1/2/3 form shapes. Caught by `ErrorBoundary`, but blocked login on any browser running auto-translate.
- **Root cause**: page declared `lang="en"` but mixed English (`USERNAME`, `PASSWORD`, `ACCESS SYSTEM`) with Vietnamese error strings (`Tài khoản hoặc mật khẩu không chính xác.`, `Mật khẩu xác nhận không khớp.`). Chrome/Edge auto-translate detected the mismatch, wrapped the visible English text in `<font>` elements, and silently took ownership of those nodes. On next React re-render, the reconciler asked the browser to remove a tracked child whose actual parent was now the translator-inserted `<font>` — DOM threw. Not a React/handler logic bug; a host-environment DOM-ownership conflict that any mixed-language UI is exposed to (settings, alarms, attendance pages all carry the same risk).
- **Fix**: opt the whole frontend out of browser translation via document-level directives. `translate="no"` is the HTML5 standard signal (Chrome/Edge/Firefox honor it via cascade to all descendants); the legacy `<meta name="google" content="notranslate">` is defense-in-depth for older Chrome / embedded WebViews. Correct choice for a security/admin console — operator's i18n is server-side, browser shouldn't be guessing.
- **Concurrency notes**: N/A (DOM ownership bug, single-threaded). One subtlety: even with both directives set, a user can still manually invoke "Translate this page" from the browser menu — at that point we're back in the same hole. A future hardening would wrap every bare text child of `<button>` and `<div className="login-error">` in `<span>` so React's reconciler can detach the span (a tracked child) instead of the text node (untracked by React DevTools internals when the parent changes shape). Not done in this pass.
- **Verification**: visual smoke test post-fix — login form renders, button state transitions (idle → loading → error → step 2) no longer trigger ErrorBoundary. No build / lint required (HTML-only change).
- **Detection lesson**: when a React `removeChild` / `insertBefore` error fires with a stack ending in a DOM element (not a React component name), 95% of the time something outside React is mutating that subtree — translators, password managers, ad blockers, screen-reader extensions. Symptom is always re-render-triggered, not first-mount, because the foreign DOM mutation needs a React reconcile pass to collide with. Repo-wide grep for `removeChild` / `notranslate` confirmed no prior fix — this is the first instance, and the index.html change immunizes the rest of the app from the same class of bug going forward.

## 2026-05-19 BUG-PPE-PHANTOM-FEATURE closed — wired producer, fixed parser, rebuilt engine

- **Files**: `cpp-backend/src/ai/inference/multi_model_infer.{h,cpp}` (Config gains 5 PPE fields + ppe_objects result vector + parallel infer fan-in), `cpp-backend/src/ai/inference/advanced_infer.{h,cpp}` (InferConfig.num_classes; parser drops hardcoded `num_classes=80` + `output_dims=84`), `cpp-backend/src/ai_worker_v2/main.cpp` (cmdline `{"ppe":true}` flag, `kPPELabels[]` 6 entries, class_id remap 0-5 → 300-305, person↔PPE containment-based violation derivation emitting synthetic class_id 406/407), `cpp-backend/src/ai/ai_event_processor.cpp` (dispatch on 406/407 → new `processPPEViolation` handler with 50px center-bucket cooldown), `cpp-backend/src/ai/inference/models/PPE-YOLOv8m.engine` (rebuilt FP16 52.6 MB from `best.pt` 155 MB).
- **Bug**: Three failures stacked. (1) PPE was a phantom feature: 55 MB engine deployed, backend had `handlePPEViolation`, frontend `PpeMonitorView` rendered "YOLOv11 PPE Active" badge — but `ai_worker` had ZERO PPE inference path. PPEDetector was deleted 2026-05-08 (commit `ed4be5c`) without also removing the receiver/UI scaffold. Compliance rate always 100% because no producer existed. (2) After producer wired, `AdvancedInfer::detectObjects` hardcoded `num_classes=80` + `output_dims=84` (COCO YOLOv11 assumption); the PPE engine's actual output size 84,000 = 8,400 anchors × 10 dims (4 box + 6 cls) failed the "Invalid output size" check OR walked past valid tensor reading garbage — every detection silently rejected. Any non-COCO TRT engine would have hit this. (3) After parser fixed (`num_classes=6`), PPE-DIAG showed raw=0 even when COCO YOLO simultaneously reported 4 persons in the same frame. Engine itself was bad — standalone test via Ultralytics TRT runtime on the engine confirmed 0 detections at conf=0.10 AND missing class metadata (engine's `metadata.json` sidecar had no class names — Ultralytics fell back to `"class0".."class999"`).
- **Detection**: feature audit before this session listed PPE as half-implemented (BUG-PPE-PHANTOM-FEATURE was already known). Live test after wiring exposed bug (2) via "Invalid output size" in `cpp_backend.log`. Diagnostic logging (PPE-DIAG histogram every 30 calls) added in commit `2f9b1ea` exposed bug (3) — raw=0 across all 6 classes even on populated frames. Standalone smoke test (`tools/test_ppe_engine.py` using Ultralytics, commit `0c3c0e3`) localized bug (3) to the engine binary specifically rather than cpp-backend preprocessing.
- **Fix**:
  1. **Producer wired** (commit `c5e3b41`): `MultiModelInfer::init()` creates parallel `AdvancedInfer` for PPE if `enable_ppe=true`. `infer()` runs PPE in parallel with fire. `ai_worker_v2/main.cpp` cmdline accepts `{"ppe": true|false}`; PPE engine class_ids 0-8 remapped to 300-308 before pushing to per-frame `metadata.objects` channel (avoids COCO namespace collision on class_id 0 = person). `ai_event_processor::processPPEViolation` emits `Event(type=PPE_VIOLATION)` with snapshot, cooldown by camera + class + 50px center grid (PPE has `track_id=-1` so location bucket is the only signal).
  2. **Parser unhardcoded** (commit `2f9b1ea`): `InferConfig.num_classes` (default 80 for COCO back-compat); `detectObjects` uses `config_.num_classes`; `output_dims = 4 + num_classes`. COCO label lookup only when `num_classes==80`; for custom engines the label is left empty for the consumer to map (ai_worker's `kPPELabels[]` does the PPE-specific naming).
  3. **Engine rebuilt + class mapping corrected** (commit `1c41524`): operator-side `convert_ppe_engine.py` rebuilt `best.pt` → ONNX → TRT FP16 (52.6 MB, 226s on RTX 4060 Laptop GPU). New engine ships 6 DETECTION-only classes: `[helmet, vest, person, gloves, mask, boots]` — NO direct NoHelmet/NoVest classes (commits 2-3 assumed there were, wrong). Violation derivation logic added in `main.cpp`: for each "person" bbox, check whether any "helmet"/"vest" bbox is CONTAINED (intersection / smaller_area ≥ 50%); on containment-fail emit synthetic class_id 406/407 detection with bbox=person's. `ai_event_processor` dispatch now keys on 406/407 (not 302/304 from the previous wrong-class-mapping commit).
- **Concurrency notes**: `MultiModelInfer::infer()` runs YOLO + Fire + PPE in parallel (existing pattern from fire wiring). PPE pipeline is single-threaded per ai_worker instance; the violation derivation loop is O(persons × ppe_objects) per frame which is bounded ≤ few dozen iterations. `ai_event_processor::processPPEViolation` dispatched via existing worker pool; cooldown map uses the same `event_cooldowns_` mutex as other dispatch handlers.
- **Class ID namespace map** (post-pass): 0-79 COCO; 100 Fire; 101 Smoke; 200 LPR; 300-305 PPE raw detection; 406/407 PPE derived violations (NoHelmet/NoVest). The 400-range is reserved for synthetic violations derived from 300-range raw detections.
- **Verification**: `cmake --build` clean + `ctest -C Release` 9/9 each commit. 10 real `PPE violation: camera=2 kind=no_helmet conf=0.50-0.57` events in `cpp_backend.log` between 20:31-21:53 confirm end-to-end pipeline (engine → parser → metadata → dispatch → Event row). Operator walked in frame without hard hat; violation events fired.
- **What's NOT closed in this pass** (deferred):
  - Per-violation IoU pairing with `track_id`: current `track_id=-1` because PPE objects pass through MultiModel before the tracker. Locking a violation to a specific tracked person would let cooldown bucket by track instead of 50px grid.
  - Compliance rate calculation: `PpeMonitorView`'s "100 - violations*2" placeholder unchanged. Real number needs `(compliant_persons / total_persons)` over rolling window.
  - `kPPELabels[]` is a C++ array; operator retraining with different class order requires source edit + rebuild. A `backend.yaml ppe.class_labels:` block would let them swap order without touching code.
  - `zmq_event_bridge::handlePPEViolation` (line 413) is now dead code for the metadata-channel approach but kept intact for an alternate signed-event path operator may wire externally.
- **Detection lesson**: when a feature is end-to-end dead, isolate the producer with a synthetic input BEFORE chasing dispatch / threshold / preprocessing. The standalone smoke test in step (3) took 30 minutes and immediately pinpointed the engine; if we'd written it first, we'd have skipped substantial "fix the cpp-backend" investigation. Three layers masked each other (no producer → wrong parser → bad engine); peeling them top-down would not have worked.

## 2026-05-19 BUG-UNIVIEW-LOOP-01 closed — bounded retry + shutdown gate on pullEvents

- **Files**: `cpp-backend/src/brand/uniview/uniview_core.cpp` (pullEvents outer loop reads `vms::core::shutting_down`, caps 5 consecutive failures, exponential 2s→30s backoff).
- **Bug**: `UniviewCore::pullEvents` had `bool running = true` outer loop never mutated. Every `streamGet` exit (camera reboot, auth expired, network drop, server-side disconnect, `onEvent` returning false) slept 5s and retried forever. Thread leak per camera that subscribed to a Uniview event stream — same anti-pattern fixed in Axis / ONVIF / Dahua / Hanwha during the 2026-05-12 brand-events sprint, but Uniview was overlooked at the time.
- **Detection**: grep for `bool running = true` across brand drivers during a periodic sweep for the 2026-05-12 pattern surfaced this last remaining instance.
- **Fix**: read `vms::core::shutting_down` each iteration and exit on set; bound consecutive failures to 5 before logging "caller may relaunch" (CameraEventService applies its own outer backoff per camera, so we don't loop forever inside one brand-core call); exponential backoff 2s → 30s ceiling between retries (was a fixed 5s, which also hammered authentication retries against a misconfigured camera); WARN log on every retry with host + failure count so operators have a breadcrumb for "Uniview camera 4 keeps reconnecting".
- **Concurrency notes**: `streamGet`'s signature is `void` — cannot distinguish clean disconnect from error exit at this level. Treating every return as a retry-worthy event is the right semantics given that the underlying transport already drops on auth or network failure; the cap prevents an auth-misconfigured camera from spinning forever.
- **Verification**: `cmake --build` clean + `ctest -C Release` 9/9 pass.
- **Detection lesson**: when fixing a pattern across N brand drivers, sweep with a regex (`bool running = true` or equivalent) for stragglers. Driver-by-driver fix is error-prone when each driver lives in its own file.
...
- **Bug**: BUG-WS-CAMERA-NO-RBAC-01 (closed `4d310688`) wired `allowed_cameras` enforcement at subscribe time but intentionally left camera_id=0 (global channel) open for any authed user so the frontend could keep receiving system-wide events. `broadcastEvent` was unchanged: every per-camera event fanned out to BOTH `camera_clients_[camera_id]` AND `camera_clients_[0]`. A non-admin viewer subscribed only to camera 0 still received the full `event.dump()` payload of EVERY camera's events — including events for cameras outside their `allowed_cameras` scope. Re-leaks the same multi-tenant PII that the subscribe-side fix tried to close, just over a different code path. Frame fan-outs (`broadcastRawFrame` / `broadcastH264Frame` / `broadcastFmp4Fragment`) were already safe — they fan out only to camera-specific subscribers via `forEachClient(camera_id, …)`, never camera 0. So the bug surface was JSON event broadcasts only.
- **Detection**: documented HIGH-MED in 2026-05-15 WS audit memo + decisions.md as the explicit follow-up to the subscribe-side RBAC pass. User priority queue. Producer survey via `grep -rn broadcastEvent` confirmed 5 call sites (EventManager, AiEventProcessor, ZmqEventBridge, alert_delivery, processBinaryMessage) all follow the "if(camera_id>0) broadcastEvent(camera_id); broadcastEvent(0);" duplicate-fan-out shape, so a single fix in `broadcastEvent` covers them all.
- **Fix**:
  1. Helper `isSocketAllowedForCamera(QWebSocket*, int)` in the anon namespace: takes `g_allowed_cameras_mutex` only; admin / auth-disabled sockets are encoded by absence from the map (subscribe-side fix never inserts them) so the helper short-circuits "allow" for them WITHOUT touching `QObject::property`. Pre-AUTH sockets are also absent but cannot reach `camera_clients_[0]` (subscribe requires AUTH), so the "absent ⟹ allow" branch is never hit from a pre-AUTH socket.
  2. `broadcastEvent`: when `camera_id == 0` (system-wide event) fan out to all global subscribers as before. When `camera_id != 0`, take the map mutex ONCE, pre-compute the allowed recipient vector, release the mutex, then `sendText` outside the lock. The lock is held purely for hashmap+set lookups; `sendText` (QueuedConnection invoke into the socket's thread) runs lock-free.
  3. `g_allowed_cameras` doc comment updated: removed the "GLOBAL-FANOUT leak remains documented" note and added the contract for `isSocketAllowedForCamera` (admin encoded by absence).
- **Concurrency notes**: producer threads for `broadcastEvent` are arbitrary (EventManager dispatcher, AI event processor, ZMQ bridge, BackgroundJobRunner workers for alert_delivery). Using a Qt property would have required `QObject::property` from those threads, which is not formally thread-safe; using a `std::mutex`-protected map is the safe-by-default choice. Lock hold time is one hashmap lookup + one set lookup per global subscriber, then released BEFORE the `QMetaObject::invokeMethod` send chain — important because the send lambda runs on the socket's thread and that thread also takes `g_allowed_cameras_mutex` during AUTH and socketDisconnected. Holding across send would risk a producer↔socket-thread deadlock if the socket thread were waiting on the producer for anything. The pre-compute pattern is the standard cure.
- **Performance**: event broadcast rate is ~1-10/sec aggregate across all events; per call the added cost is `O(globally subscribed clients)` mutex+lookup. At <100 global subscribers this is microseconds, no measurable impact vs the JSON serialisation already in the function. Frame broadcasts (the actual hot path at 30 fps × N cameras) are unaffected — they don't use camera 0.
- **What this does NOT close**: `processBinaryMessage` path. If a non-admin client sends a binary WS frame with `{camera_id: N, …}`, `broadcastEvent(N, j)` is called directly. The camera-specific fan-out at line 731-735 hits only `camera_clients_[N]` — and the subscribe-side RBAC fix already gates that. So binary-event injection by a client only reaches subscribers who could legitimately receive events for camera N. Confirmed safe.
- **Verification**: `cmake --build` clean, `ctest -C Release` 9/9 pass, controller-lint guard reports zero antipattern violations.
- **Detection lesson**: when a security fix "intentionally allows" a wildcard for legitimate-use-case reasons, the wildcard becomes the new attack surface. Always trace the producer path of whatever flows through that wildcard. Here the wildcard was camera-0 subscribe; the leak was at the broadcast emitter. Schema-level RBAC enforcement is only half the work — every emitter that touches the wildcard channel needs a matching scope check, OR the schema needs to enforce "no per-scope data on the wildcard channel" (e.g., separate global-system-event channel from per-camera fan-out). We chose emitter-side filtering because the second option would have broken existing frontend dependencies on the global channel.
