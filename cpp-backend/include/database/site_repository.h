...
public:
    ...
    static bool updateSite(const Site& site) {
        auto& db_mgr = DbManager::getInstance();
        return db_mgr.executeParameterized(
            "UPDATE sites SET name=?, address=?, parent_site_id=?, description=? WHERE id=?",
            {site.name, site.address, std::to_string(site.parent_site_id),
             site.description, std::to_string(site.id)});
    }
