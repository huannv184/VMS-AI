...
public:
    ...
    static bool setPermissions(const UserPermission& p) {
        ...
        std::string q = db_mgr.isPostgres()
            ? "INSERT INTO permissions (user_id, allowed_cameras, allowed_sites, can_view_live, can_view_playback, can_manage_settings, can_ptz) VALUES (?, ?, ?, ?, ?, ?, ?) ON CONFLICT (user_id) DO UPDATE SET allowed_cameras=EXCLUDED.allowed_cameras, allowed_sites=EXCLUDED.allowed_sites, can_view_live=EXCLUDED.can_view_live, can_view_playback=EXCLUDED.can_view_playback, can_manage_settings=EXCLUDED.can_manage_settings, can_ptz=EXCLUDED.can_ptz"
            : "INSERT OR REPLACE INTO permissions (user_id, allowed_cameras, allowed_sites, can_view_live, can_view_playback, can_manage_settings, can_ptz) VALUES (?, ?, ?, ?, ?, ?, ?)";
