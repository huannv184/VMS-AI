...
// ── PPE compliance aggregator impl moved to ppe_compliance_aggregator.cpp
//    on 2026-05-25 (PR-7B refactor — behaviour-preserving). The class is
//    still visible here via the transitive #include in
//    ai_event_processor.h, so eventWorkerLoop's PpeComplianceAggregator::
//    getInstance().recordTick(...) call above continues to compile.
// ────────────────────────────────────────────────────────────────────────────
...
void AiEventProcessor::processPPEViolation(int camera_id,
    ...
    std::string kind;
    std::string desc;
    if (class_id == 406 || label == "NoHelmet") {
        kind = "no_helmet";
