...
TrafficSummary TrafficRepository::getSummary(int camera_id) {
    ...
    // strftime/datetime are SQLite-only — Postgres needs to_char(to_timestamp...)
    // to extract the hour from an epoch column. Without the branch this query
    // would throw "function datetime(bigint, unknown) does not exist" on PG.
    {
        const std::string hr_expr = DbManager::getInstance().isPostgres()
            ? "to_char(to_timestamp(period_start), 'HH24')"
            : "strftime('%H', datetime(period_start, 'unixepoch'))";
        const std::string sql =
            "SELECT " + hr_expr + " as hr, SUM(count) as total "
            "FROM traffic_counts WHERE camera_id = ? AND period_start >= ? "
